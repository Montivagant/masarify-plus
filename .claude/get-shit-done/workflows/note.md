...
- **Global scope**: `D:/Masarify-Plus/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Masarify-Plus/.claude/notes/`)
...
2. Glob `D:/Masarify-Plus/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/Masarify-Plus/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Masarify-Plus/.claude/notes/` — works in any directory
